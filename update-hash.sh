...
DIR_ASSIGNMENT_4=/home/user/EmbeddedLinux/buildroot-avt82/
PKG_NAME=aesd-assignments
EXT_PKG_DIR=${DIR_ASSIGNMENT_4}base_external/
...
# converts $1 to uppercase, replacing '-' with '_'
function uppercase_underscore() {
    echo -en "${1^^}" | tr '-' '_'
}
...
function process_mk_file() {
    ...
    MK_FILE=${EXT_PKG_DIR}package/${1}/${1}.mk

    if [ ! -f "${MK_FILE}" ] ; then
	echo "Cannot find file ${MK_FILE}"
	return 2
    fi
    ...
    SEARCH=`uppercase_underscore "${1}-version"`
    # gets the file with altered string with hash


    new_text=`cat_file_altered "${MK_FILE}" "${SEARCH}" "${SEARCH} = '${new_hash}'"`

    ret=$?
    if (( ret > 0 )) ; then
	backup_file "${MK_FILE}" "${MK_FILE}.old"
	echo " - Replacing file ${MK_FILE}..."
	echo "${new_text}" > "${MK_FILE}"
	return 0
    fi
    echo " - No hash to update in ${MK_FILE}..."
